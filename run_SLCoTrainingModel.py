...
from source.model.SLCoTrainingModel import SLCoTrainingModel
...
def fit(params):
    for fold in params.data.folds:
        ...
        # Train the ⚡ model
        trainer.fit(
            model=SLCoTrainingModel(params.model),
            datamodule=SLDataModule(
                params.data,
...
def predict(params):
    for fold in params.data.folds:
        ...
        # model
        model = SLCoTrainingModel.load_from_checkpoint(
