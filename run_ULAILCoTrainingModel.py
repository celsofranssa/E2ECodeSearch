...
from source.model.ULAILCoTrainingModel import ULAILCoTrainingModel
...
def fit(params):
    for fold in params.data.folds:
        ...
        # Train the ⚡ model
        trainer.fit(
            model=ULAILCoTrainingModel(params.model),
...
def predict(params):
    for fold in params.data.folds:
        ...
        # model
        model = ULAILCoTrainingModel.load_from_checkpoint(
        ...
        trainer = pl.Trainer(
            gpus=params.trainer.gpus,
            callbacks=[ULPredictionWriter(params.prediction)]
        )
